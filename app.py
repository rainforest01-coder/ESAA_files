    ...
    if st.button("📊 진단 결과 확인", use_container_width=True):
        ...
        if target_value == 0:
            ...
            if q1_age >= 60:
                ...
                one_line_desc = "뉴스를 이용할 때 출처를 확인하는 습관을 조금 더 기르면 정보의 신뢰성을 판단하는 데 도움이 됩니다."
                alert_style = st.warning
            elif g3_thumbnail_flag == 1:
                user_type = "썸네일 중심 이용형"
                one_line_desc = "제목이나 썸네일의 영향을 비교적 크게 받는 뉴스 이용 패턴을 보입니다. 눈길을 사로잡는 낚시성 헤드라인에 넘어가지 않게 주의가 필요합니다."                
                alert_style = st.error
            elif g11_val in [1, 2]:
                user_type = "출처 확인 미흡형"
                one_line_desc = "뉴스를 이용할 때 언론사와 출처를 함께 확인하는 습관을 권장합니다."
                alert_style = st.error
            elif g4_5_val in [4, 5]:
                user_type = "공유 빈도 높은 이용형"
                one_line_desc = "뉴스를 공유하기 전에 다른 언론사의 보도와 함께 확인하면 정보의 신뢰성을 높일 수 있습니다."
                alert_style = st.error
            else:
                user_type = "일반 이용형"
                one_line_desc = "평소 뉴스를 이용할 때 출처 확인과 교차 검증을 함께 하면 더욱 신뢰도 높은 정보 이용에 도움이 됩니다."
                alert_style = st.warning
...
# ------------------------------------------------------------------------------
with tab2:
    st.subheader("🚀 유튜브 허위 선동성 실시간 검증 시스템")
    st.markdown("""
    최근 미디어 정보 취약 계층을 노리고 공포, 분노, 조바심을 자극하여 조회수를 유도하는 유튜브 콘텐츠가 급증하고 있습니다.  
    검증하고 싶은 **유튜브 영상 링크**를 입력하시면 AI가 제목, 본문, 댓글 트렌드를 실시간 분석하여 리포트를 제공합니다.
    """)
